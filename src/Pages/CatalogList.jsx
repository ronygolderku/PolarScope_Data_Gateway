        ...
        const productsWithDetails = await Promise.all(
          childLinks.map(async (link) => {
            try {
              ...
              const res = await fetch(`/data/products/${productPath}/collection.json`);
              const details = await res.json();
              return {
                ...link,
                title: details.title || link.title, 
                description: details.description, 
                extent: details.extent, // Fetch extent for dynamic dates 
              };
            } catch (err) {
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6 md:space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="text-sm text-gray-500 pl-6 flex flex-wrap gap-2 items-center">
          <span>in <span className="text-primary font-semibold">PolarScope Catalog</span></span>
          <span className="hidden md:inline mx-2">|</span>
             <button className="btn btn-xs btn-outline rounded-sm">Up</button>
             <button className="btn btn-xs btn-outline rounded-sm">Overview</button>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="flex-1 flex flex-col">
           ...
           <div className="mt-4 order-2 lg:order-none">
             <h3 className="text-xl font-bold text-secondary mb-2">Additional Resources</h3>
              <ul className="list-disc list-inside text-sm pl-2">
                  <li><a href="#" className="text-blue-600 hover:underline">Description</a></li>
              </ul>
           </div>
        </div>

        <div className="flex-1 flex flex-col order-3 lg:order-none">
          <div className="bg-gray-100 p-2 border border-gray-200 rounded-md shadow-sm h-full max-h-[400px]">
             <img src={getThemeImage()} alt={themeId} className="w-full h-full object-cover rounded" />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
          ...
          <div className="flex flex-col md:flex-row gap-4">
              <input type="text" placeholder="Filter catalogs by title, description or keywords" className="input input-bordered w-full" />
              <select className="select select-bordered w-full md:max-w-xs">
                  <option disabled selected>Select keywords</option>
