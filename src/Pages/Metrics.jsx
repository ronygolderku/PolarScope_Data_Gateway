...
import { FaGlobe, FaSearch, FaChevronDown, FaChevronRight, FaExternalLinkAlt, FaFilter, FaMapMarkerAlt } from 'react-icons/fa';
            ...
            <div className="bg-white rounded shadow-2xl w-full max-w-5xl flex flex-col max-h-[90vh] overflow-hidden">
                ...
                <div className="p-6 border-b border-gray-100 flex justify-between items-start">
                    <div className="flex gap-3">
                         ...
                         <div>
                             <h3 className="text-xl font-bold text-gray-800 leading-snug">{product.title}</h3>
                ...
                <div className="flex flex-col md:flex-row flex-1 min-h-0">
                    ...
                    <div className="w-full md:w-1/3 p-6 border-r border-gray-100 overflow-y-auto">
                        <h4 className="text-xs font-bold text-[#009d9a] uppercase mb-4">Products</h4>
                             ...
                             <div className="group">
                                 <div className="flex items-start justify-between gap-2">
                                     ...
                                     <div className="flex items-center gap-2 text-gray-400 shrink-0">
                                         <FaMapMarkerAlt className="hover:text-[#009d9a] cursor-pointer" size={14}/>
                                         <a href={`/products/${product.productPath}`} target="_blank" rel="noreferrer">
                                             <FaExternalLinkAlt className="hover:text-[#009d9a] cursor-pointer" size={12}/>
                                         </a>
                                     </div>
                                 </div>
                             </div>
                    ...
                    <div className="w-full md:w-2/3 bg-gray-50 relative min-h-[400px]">
                        ...
                        >
                            ...
                            />
                            {bbox && (
                                <Rectangle bounds={bounds} pathOptions={{ color: '#009d9a', weight: 2, fillOpacity: 0.1, dashArray: '5, 5' }} />
                            )}
                        </MapContainer>
                    </div>
                </div>
...
const ProductRow = ({ product, minYear, maxYear, totalYears, isExpanded, onToggle, onOpenModal }) => {
                ...
                <div className="bg-gray-50 p-4 border-t border-gray-100 pl-[40px] shadow-inner">
                    <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <div className="w-64 space-y-2">
                            <div className="text-xs font-bold uppercase text-gray-400">Metadata</div>
                             <div className="text-xs text-gray-600 grid grid-cols-2 gap-x-2 gap-y-1">
                                ...
                                <span className="font-semibold">Region:</span> {product["osc:region"] || "N/A"}
                                <span className="font-semibold">Project:</span> {product["osc:project"] || "N/A"}
                             </div>
                        </div>
                    </div>
                </div>
...
const Metrics = () => {
    ...
    // GAP 4 YEARS
    const YEARS_ARRAY = Array.from({length: Math.ceil(TOTAL_YEARS / 4)}, (_, i) => MIN_YEAR + (i * 4)); 
        ...
        const fetchData = async () => {
            try {
                ...
                const allProductsProms = themeLinks.map(async (themeLink) => {
                    ...
                    const productLinks = themeCatalog.links.filter((link) => link.rel === 'child');
    ...
    return (
        <div className="flex flex-col min-h-screen bg-white text-gray-800">
                ...
                <div className="max-w-[1920px] mx-auto px-4 md:px-8 py-4 space-y-4">
                     ...
                     <div className="flex items-center justify-between">
                         <div className="flex items-center gap-4">
                             <h1 className="text-2xl font-bold uppercase tracking-tight text-[#003049]">Metrics</h1>
                     ...
                     <div className="flex flex-wrap items-center gap-4">
                         ...
                         <select className="select select-sm select-bordered rounded-sm text-xs w-32 bg-white">
                             <option disabled selected>Coverage</option>
                                 ...
                                 {YEARS_ARRAY.map(year => {
                                      ...
                                      return (
                                          <div 
                                            ...
                                            style={{ left: `${left}%` }}
                                          >
                                              <span className="pl-1 text-[10px] opacity-70">{year}</span>
                                          </div>
                                      );
                                 })}
                             </div>
             ...
             {/* Modal */}
             {selectedProduct && <CoverageModal product={selectedProduct} onClose={() => setSelectedProduct(null)} />}
        </div>
    );
};
