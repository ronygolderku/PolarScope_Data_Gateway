    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="space-y-2">
        ...
        <div className="text-sm text-gray-500 pl-6 flex flex-wrap gap-2 items-center">
          <span>
            in <span className="text-secondary font-semibold">Open Science Catalog</span>
          </span>
          <span className="hidden md:inline mx-2">|</span>
          <button onClick={() => navigate(-1)} className="btn btn-xs btn-outline rounded-sm">
            Up
          </button>
          <button onClick={() => navigate(`/themes/${productPath.split('-')[0]}`)} className="btn btn-xs btn-outline rounded-sm">
            Overview
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-8">
          {/* Description */}
          <div>
            <h2 className="text-xl font-bold text-secondary mb-3">Description</h2>
            ...
            {data.keywords && (
              <div className="flex flex-wrap gap-2 mt-4">
                {data.keywords.map((k, i) => (
                  <span key={i} className="badge badge-neutral text-xs rounded-sm">
                    {k}
                  </span>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mt-4">
             <div>
                <span className="font-bold">License</span>: {data.license || "Unknown"}
             </div>
             <div>
                 <span className="font-bold">Temporal Extent</span>: {data.extent?.temporal?.interval?.[0]?.[0] ? new Date(data.extent.temporal.interval[0][0]).toLocaleDateString() : "Unknown"} - {data.extent?.temporal?.interval?.[0]?.[1] ? new Date(data.extent.temporal.interval[0][1]).toLocaleDateString() : "Present"}
             </div>
          </div>
          ...
          <div>
               <h3 className="text-lg font-bold text-blue-900 mb-2">Related EarthCODE Forum Topics</h3>
               <p className="text-sm text-gray-500 italic">Didn't find what you were looking for? <a href="#" className="font-semibold text-blue-600 hover:underline">Start a new topic on the EarthCODE forum!</a></p>
          </div>
        </div>
        ...
        <div className="lg:w-1/3 space-y-8">
          ...
          <div>
            ...
            <div>
                <h3 className="font-bold text-gray-700 mb-2">Open Science Catalog</h3>
                <div className="overflow-x-auto border border-gray-200 rounded">
                    <table className="table table-xs w-full">
                         <tbody>
                             <tr className="bg-gray-50 border-b border-gray-100">
                                 <td className="font-semibold w-24">Project</td>
                                 <td className="text-blue-600 font-semibold">{data["osc:project"] || "Unknown"}</td>
                             </tr>
          ...
          <div>
               ...
               <div className="text-sm space-y-2">
                   <div><span className="font-semibold">Related resource</span></div>
                   <ul className="list-disc list-inside pl-2 text-gray-600">
                      
                       <li>Project: {data["osc:project"] || "Unknown"}</li>
                      
                       <li>Theme: {data.themes?.find(t => t.concepts?.[0]?.id)?.concepts[0].id || "Unknown"}</li>
                       <li>EO Mission: {data["osc:missions"]?.[0] || "N/A"}</li>
                   </ul>
                   <div className="mt-2"><span className="font-semibold">Source metadata</span></div>
                   <ul className="list-disc list-inside pl-2 text-gray-600">
                        {data.links?.filter(l => l.rel === "via" || l.rel === "self").map((link, idx) => (
                            <li key={idx}><a href={link.href} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">{link.title || "Link"}</a></li>
                        ))}
                   </ul>
               </div>
          </div>
